-- Migration 0015: Customer Credit Enforcement and Due Dates

-- Safely drop old process_sale signatures to prevent ambiguity
DROP FUNCTION IF EXISTS public.process_sale(UUID, UUID, JSONB, JSONB);
DROP FUNCTION IF EXISTS public.process_sale(UUID, UUID, JSONB, JSONB, TIMESTAMPTZ);

-- Recreate with p_due_date
...
BEGIN
    ...
    -- Create Sale
    INSERT INTO public.sales (store_id, organization_id, customer_id, cashier_id, status, subtotal, discount_total, tax_total, grand_total, due_date)
    VALUES (p_store_id, v_org_id, p_customer_id, auth.uid(), 'COMPLETED', v_subtotal, v_discount_total, v_tax_total, v_grand_total, p_due_date)
