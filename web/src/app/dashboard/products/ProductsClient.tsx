...
export default function ProductsClient({ 
  ...
  // Effect to check duplicates when SKU or Barcode changes
  useEffect(() => {
    const timer = setTimeout(async () => {
      if ((sku.trim().length > 0) || (barcode.trim().length > 0)) {
        setIsCheckingDuplicate(true)
        const res = await checkDuplicateProduct(barcode, sku)
        setIsCheckingDuplicate(false)
        if (res.duplicate && res.product) {
          setDuplicateWarning({
              ...
              <form onSubmit={handleFormSubmit} className="flex-1 p-6 overflow-y-auto">
                    ...
                    {openingStock > 0 && (
                      <div className="mt-3 p-3 bg-indigo-50 text-indigo-700 rounded-md text-sm">
                        ...
                        <span className="font-semibold">
                          ...
                          {packagingType !== 'NONE' && ` x ${unitsPerPack} units`} = {openingStock * (packagingType !== 'NONE' ? unitsPerPack : 1)} total base units
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="mt-6 flex flex-col gap-3 pt-4 border-t border-gray-200 sticky bottom-0 bg-white pb-2 z-10">
                  ...
                  <div className="flex justify-end gap-3">
                    ...
                    <button type="submit" disabled={isPending || isScanning || isCheckingDuplicate} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:bg-indigo-400">
